...
export default function GerenciarTemas() {
  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={salvarTema} className="bg-white p-4 rounded-lg shadow border space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <input name="slug" value={form.slug} onChange={handleChange} placeholder="Slug (ex: moderno)" className="border p-2 rounded" required />
          <input name="descricao" value={form.descricao} onChange={handleChange} placeholder="Descrição" className="border p-2 rounded" />
          <input name="preview_image_url" value={form.preview_image_url} onChange={handleChange} placeholder="URL da imagem de prévia" className="border p-2 rounded" />
        ...
        <div>
          <label className="block text-sm font-medium">Configuração (JSON)</label>
          <textarea name="config" value={form.config} onChange={handleChange} rows={4} className="w-full border p-2 rounded font-mono text-sm" />
        </div>
        <div className="flex gap-2">
          ...
          {editando && (
            <button type="button" onClick={() => { setEditando(null); resetForm() }} className="bg-gray-300 px-4 py-2 rounded">
              Cancelar
            </button>
          )}
        </div>
        ...
        {temas.map((tema) => (
          <div key={tema.id} className="bg-white p-4 rounded-lg shadow border">
            ...
            <div className="flex items-center gap-2 mt-1">
              <span className={`text-xs px-2 py-0.5 rounded ${tema.tipo === 'premium' ? 'bg-amber-100 text-amber-800' : 'bg-gray-100'}`}>
                {tema.tipo === 'premium' ? '🔒 Premium' : 'Grátis'}
              </span>
              <span className={`text-xs px-2 py-0.5 rounded ${tema.ativo ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                {tema.ativo ? 'Ativo' : 'Inativo'}
              </span>
            </div>
            <div className="mt-2 flex gap-2">
              <button onClick={() => editarTema(tema)} className="text-blue-600 text-sm hover:underline">Editar</button>
              <button onClick={() => deletarTema(tema.id)} className="text-red-600 text-sm hover:underline">Remover</button>
            </div>
          </div>
        ))}
      </div>
