...
export function GerenciadorPlanos({ planos, temas, modelosQR, recursos, onUpdate }) {
  ...
  const atualizarPlanoLocal = (planoId, atualizacao) => {
    const novosPlanos = planos.map(p => p.id === planoId ? { ...p, ...atualizacao } : p)
    onUpdate(novosPlanos)
    return novosPlanos
  }

  const toggleTemaPlano = async (planoId, temaSlug, marcado) => {
    const plano = planos.find(p => p.id === planoId)
    const temasAtuais = plano.temas_permitidos || []
    const novosTemas = marcado ? temasAtuais.filter(t => t !== temaSlug) : [...temasAtuais, temaSlug]
  ...
  const toggleModeloQRPlano = async (planoId, modeloSlug, marcado) => {
    const plano = planos.find(p => p.id === planoId)
    const modelosAtuais = plano.modelos_qrcode_permitidos || []
    const novosModelos = marcado ? modelosAtuais.filter(m => m !== modeloSlug) : [...modelosAtuais, modeloSlug]
  ...
  const toggleRecursoPlano = async (planoId, recursoSlug, marcado) => {
    const plano = planos.find(p => p.id === planoId)
    const recursosAtuais = plano.recursos_permitidos || []
    const novosRecursos = marcado ? recursosAtuais.filter(r => r !== recursoSlug) : [...recursosAtuais, recursoSlug]
  ...
  const alterarLimitePlano = async (planoId, novoLimite) => {
  ...
  const excluirPlano = async (id, nome) => {
    const { count } = await supabase.from('estabelecimentos').select('*', { count: 'exact', head: true }).eq('plano_id', id)
    if (count > 0) return alert(`Não é possível excluir "${nome}" porque há ${count} estabelecimento(s) vinculado(s).`)
