...
export function CardapioTab({
  ...
  const handleSaveLayout = async () => { if (layoutTemp !== layoutAtual) { setLayoutAtual(layoutTemp); await onSalvarLayout(layoutTemp) } }
  const handleSaveTema = async () => { if (temaTemp !== temaAtual) { setTemaAtual(temaTemp); await onSalvarTema(temaTemp) } }
  const handleSaveBackground = async () => { if (bgTemp !== bgAtual) { setBgAtual(bgTemp); await onSalvarBackgroundImage(bgTemp) } }
    ...
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="flex items-center gap-1">
            <select value={layoutTemp} onChange={(e) => setLayoutTemp(e.target.value as any)} className="border rounded-lg px-2 py-1.5 text-sm bg-white">
            ...
            <button onClick={handleSaveLayout} disabled={layoutTemp === layoutAtual} className={`px-3 py-1.5 rounded-lg text-sm font-medium transition ${layoutTemp === layoutAtual ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'bg-orange-600 text-white hover:bg-orange-700'}`}>Salvar</button>
          </div>
          <button onClick={onNovaCategoria} className="border-2 border-gray-300 text-gray-700 px-3 py-1.5 rounded-lg text-sm font-semibold hover:bg-gray-100">➕ Nova Categoria</button>
        </div>
      </div>
        ...
        <div className="bg-white rounded-xl p-4 shadow-sm mb-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 mb-4">
            {temasDisponiveisFiltrados.map((tema) => (
              <TemaCard key={tema.id} tema={tema} selecionado={temaTemp === tema.slug} onClick={() => setTemaTemp(tema.slug)} />
            ))}
          </div>
          <button onClick={handleSaveTema} disabled={temaTemp === temaAtual} className={`px-4 py-2 rounded-lg text-sm font-medium transition ${temaTemp === temaAtual ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'bg-orange-600 text-white hover:bg-orange-700'}`}>Salvar tema</button>
        </div>
      )}
      ...
      <div className="bg-white rounded-xl p-4 shadow-sm mb-6">
        ...
        <div className="flex flex-wrap gap-4 items-end">
          <div className="flex-1"><ImageUpload onUpload={(url) => setBgTemp(url)} defaultImage={bgTemp} /></div>
          <button onClick={handleSaveBackground} disabled={bgTemp === bgAtual} className={`px-4 py-2 rounded-lg text-sm font-medium transition ${bgTemp === bgAtual ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'bg-orange-600 text-white hover:bg-orange-700'}`}>Salvar fundo</button>
        </div>
        {bgAtual && <div className="mt-3 text-xs text-gray-500">Fundo atual: <a href={bgAtual} target="_blank" className="text-blue-600 underline">Visualizar</a></div>}
      </div>
